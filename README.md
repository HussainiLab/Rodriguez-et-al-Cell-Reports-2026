# Rodriguez-2026
This repo is associated with the data and for the paper by Rodriguez et al. Cell Reports 2026

It includes data in Excel format (.xlsx) listed below:
1. Data for all figures and supplemental figures
2. Spatial information scores data
3. Spatial cells data to label them isSpatial for EMD analysis
4. EMD data
5. Shuffled data for EMD analysis.

The following code was used for analysis and links Github repositories are listed below:
1. Batch sorting script that uses KlustKwik: https://github.com/HussainiLab/BatchTINTV3
2. GebaSpike for manual curation of spikes: https://github.com/HussainiLab/gebaSpike
3. Earth Mover's Distance (EMD) code: https://github.com/HussainiLab/NSK-isolated/tree/main/_prototypes/cell_remapping
4. Neuronal analysis (firing rate, spatial information, grid score, border score etc.) was done using PRISM: https://github.com/HussainiLab/PRISM
